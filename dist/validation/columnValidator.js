 ...
 */
function isInStringLiteral(query, position) {
    let inString = false;
 ...
 */
function extractTablesWithAliases(query) {
    const tables = [];
 ...
 */
function getTableColumns(tableName, schema) {
 ...
 */
function getAllColumnsInScope(tables, schema) {
 ...
 */
function tableExistsInSchema(tableName, schema) {
    return schema.tables.some((t) => t.name.toLowerCase() === tableName.toLowerCase());
}
/**
 * Extract all column references from the query.
 * This is the key function that identifies where columns are used.
 */
function extractColumnReferences(query, tables) {
    const refs = [];
    ...
    while ((match = identifierRegex.exec(query)) !== null) {
        ...
        if (isInStringLiteral(query, position))
            continue;
        // Skip if this position should be skipped (table/alias definition)
        if (skipPositions.has(position))
            continue;
        // Skip SQL keywords
        if (KEYWORDS_SET.has(upperWord))
            continue;
        // Skip SQL functions (typically followed by parenthesis)
        if (FUNCTIONS_SET.has(upperWord))
            continue;
 ...
 */
export function validateColumns(query, schema) {
    if (!query.trim()) {
        return [];
    }
    const errors = [];
    ...
    for (const ref of columnRefs) {
        if (ref.table) {
            ...
            if (!allColumnsInScope.has(ref.name)) {
                ...
                const knownTables = tables.filter((t) => tableExistsInSchema(t.tableName, schema));
